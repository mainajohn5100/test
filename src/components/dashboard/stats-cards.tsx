...
import { Ticket as TicketIcon, BarChart2, FilePlus2, Briefcase, ArrowUp, ArrowDown, CheckCircle } from "lucide-react";
...
export function StatsCards({ tickets, projects }: StatsCardsProps) {
  ...
  const newTicketsLastWeek = tickets.filter(t => isWithinInterval(new Date(t.createdAt), { start: startOfLastWeek, end: endOfLastWeek })).length;
  ...
  const statItems = [
    { title: "Open Tickets", value: openTickets, icon: TicketIcon, change: newTicketsChange.change, changeType: newTicketsChange.changeType, footerText: "vs last week's new tickets", color: "text-blue-600", bgColor: "bg-blue-100", disabled: false },
    ...
    { title: "New This Week", value: newTicketsThisWeek, icon: FilePlus2, change: newTicketsChange.change, changeType: newTicketsChange.changeType, footerText: "vs last week", color: "text-purple-600", bgColor: "bg-purple-100", disabled: false },
