...
import { getOrganizationByWhatsAppNumber, getUserByPhone, addTicket, addAutoGeneratedUser, getOpenTicketsByUserId, addConversation, createNotification, getUserByName, getUsers, getRecentlyClosedSlaTicketsForUser, updateTicket } from '@/lib/firestore';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user) {
        ...
        user = { ...newUserData, id: newUserId, createdAt: newUserData.createdAt };
    ...
    // --- CSAT Handling ---
    const rating = parseInt(messageBody, 10);
    if (!isNaN(rating) && rating >= 1 && rating <= 5) {
        const recentlyClosedTickets = await getRecentlyClosedSlaTicketsForUser(user.id);
        if (recentlyClosedTickets.length > 0) {
            const ticketToRate = recentlyClosedTickets[0]; // Rate the most recently closed ticket
            if (ticketToRate.csatStatus === 'pending') {
                 await updateTicket(ticketToRate.id, {
                    csatScore: rating as 1 | 2 | 3 | 4 | 5,
                    csatStatus: 'rated'
                });
                ...
                console.log(`Recorded CSAT score of ${rating} for ticket ${ticketToRate.id}`);
                return NextResponse.json({ success: true, message: `Recorded CSAT score for ticket ${ticketToRate.id}` });
            }
        }
    }
