...
import { getOrganizationByWhatsAppNumber, getUserByPhone, addTicket, addAutoGeneratedUser, getOpenTicketsByUserId, addConversation, createNotification, getUserByName } from '@/lib/firestore';
        ...
        if (ticketToUpdate.assignee && ticketToUpdate.assignee !== 'Unassigned') {
            ...
            if (assignee) {
                 const { summary } = await summarizeNewMessage({
                    from: user.name,
                    message: messageBody,
                    ticketTitle: ticketToUpdate.title,
                });
