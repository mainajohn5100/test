...
export async function addReplyAction(formData: FormData) {
  try {
    ...
    if (!ticketId || !authorId) throw new Error('Missing required data for reply.');
    if (!content && files.length === 0) {
      return { success: false, error: 'A reply must have content or an attachment.' };
    }
    ...
    await notifyUsers({
      ...
      link: `/tickets/view/${ticketId}`,
      type: 'new_reply',
      metadata: { from: author.name, ticketTitle: ticket.title },
    });
    ...
    if (
      ...
      if (orgWhatsapp?.accountSid && orgWhatsapp?.authToken && orgWhatsapp?.phoneNumber) {
        try {
          ...
          console.log(`WhatsApp reply sent to ${ticket.reporterPhone} for ticket ${ticketId}`);
        } catch (twilioError) {
...
export async function updateTicketAction(
  ...
  try {
    ...
    if (updates.status) dataToUpdate.statusLastSetBy = user.role;
    if (updates.priority) dataToUpdate.priorityLastSetBy = user.role;
